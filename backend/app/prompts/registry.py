...
QUALITY_RULES_BLOCK = (
    "Quality rules:\n"
    "- Each question must be answerable directly from the provided document/context.\n"
    "- The prompt and the expected answer must belong to the same semantic category. "
    "For example, if the prompt asks for sources, the answer must list sources; "
    "if it asks for data types, the answer must list data types.\n"
    "- Do not ask for \"types of data\" when the document describes sources, tools, platforms, "
    "people, years, organizations, or fields.\n"
    "- For single_choice questions, all options must be the same kind of entity: "
    "all years, all names, all terms, all places, etc.\n"
    "- Do not mix a factual option with meta-options like \"not specified\", "
    "\"all of the above\", or \"none of the above\" unless the document explicitly requires it.\n"
    "- For true_false questions, write one clear statement that is directly verifiable from the document.\n"
    "- For fill_blank questions, the blank must correspond to a short phrase explicitly present in the document.\n"
    "- For short_answer questions, ask a specific question with a specific answer, "
    "not a vague prompt like \"What fact is stated in the text?\"\n"
    "- Prefer concise prompts and concise answers.\n"
)
    ...
    _registry = {
        DIRECT_GENERATION_PROMPT_KEY: PromptDefinition(
            ...
            user_template=(
                ...
                "Question type rules:\n{question_type_rules}\n"
                f"{QUALITY_RULES_BLOCK}"
                f"{MATCHING_REGISTRY_RULE}"
        ...
        REPAIR_GENERATION_PROMPT_KEY: PromptDefinition(
            ...
            user_template=(
                ...
                "If the invalid JSON has fewer questions than requested:\n"
                "- Add new grounded questions from the source document/context until the total is exactly {question_count}.\n"
                "- Use only allowed question types from the question type policy.\n"
                "- Do not stop after using each allowed question type once.\n"
                "- If more questions are needed after every allowed type appears, repeat suitable allowed question types.\n"
