...
def test_docx_exporter_builds_openable_quiz_document_with_cyrillic_content() -> None:
    ...
    assert "1. Москва" in document_text
    assert "2. Казань" in document_text
    assert "Правильный ответ: Москва" in document_text
    assert "Пояснение: Москва является столицей России." in document_text
    assert "Какая река протекает через Санкт-Петербург?" in document_text
    assert "Правильный ответ: Нева" in document_text
...
def test_default_export_registry_exposes_docx_exporter() -> None:
    ...
    assert exported_file.filename == "quiz-ru-1.docx"
    assert "Тренировочный квиз по географии" in read_docx_text(exported_file.content_bytes)
