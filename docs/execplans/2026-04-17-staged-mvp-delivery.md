...
Next recommended batch:
    ...
    Stage 15 Batch 1: provider feature flags and provider registry enforcement (`CF-004`) that introduce a typed provider registry on top of the existing `LMStudioClient`, surface enable/disable flags for each provider through `AppConfig` (for example `PROVIDERS_ENABLED=lm_studio,ollama`), enforce the flags before dispatching any generation or embedding request, and cover the new behavior with positive and negative tests including a Russian/Cyrillic generation example. Stage 15 Batch 2 should add the Ollama provider adapter (`LM-007`) implementing `LLMProvider`, including healthcheck, structured generation, and embeddings, behind the registry. Stage 15 Batch 3 should add an external-API provider adapter (`LM-008`) on the same registry contract. After Stage 15 the only remaining backlog tier is Stage 16 (`RAG-006` retrieval cache) and any cross-cutting Future-tier hardening that has not yet been promoted into a stage.
