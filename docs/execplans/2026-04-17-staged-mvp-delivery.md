...
Next recommended stage:
    ...
    Stage 14: RAG Generation (API wiring batch pending)

Next recommended batch:

    Stage 14 Batch 4: wire `RagGenerationOrchestrator` and `select_generation_mode` into the runtime generation API path so that `POST /generate` actually dispatches between the direct and RAG orchestrators based on the persisted/requested generation mode and the rule-based selector, while preserving backward compatibility for existing `direct` requests, keeping the `single_question_regen` endpoint unchanged, and updating the smoke and integration tests to cover both the `direct` and `rag` API paths with Russian/Cyrillic content. Frontend mode-selection wiring (if needed) can land in the same batch or in a follow-up frontend slice. After Stage 14 Batch 4, the next stage is Stage 15 (Additional Providers and Feature Flags), starting with Batch 1 covering the provider feature flags and the provider registry enforcement (`CF-004`), followed by the secondary provider adapters (`LM-007` Ollama, `LM-008` external API).
